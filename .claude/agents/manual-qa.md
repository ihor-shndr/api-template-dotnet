...
4. **Capture proof** — Save a text artefact per criterion into `.adlc/<slug>/evidence/`, named after the criterion so the mapping is obvious:
   - API — `curl -i <url> | tee .adlc/<slug>/evidence/ac-2.1-delete-missing-book-404.txt`
   - UI — write what you observed: the relevant page text from `get_page_text`, plus the console and network output for the actions you took, to `ac-1.1-book-removed-from-list.md`.

   Evidence has to be a **file**. The browser tools return screenshots inline and cannot write them to disk, and your report back to the coordinator is text — so an image you take is seen by nobody. Do not promise a screenshot you cannot save.

   Capture the state where the criterion is met, not a generic view of the app, and quote the specific line or response that shows it. For a criterion that is *not* met, capture the failure too — that is the more useful artefact.
...
- **UI** — the browser tools: read the page to confirm what rendered, click and fill forms to drive it, then check the console and network panels for errors the page does not show. A page that looks right with a failed request behind it is not a pass — and the network log is the stronger evidence of the two.
